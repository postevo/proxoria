...
export async function routeRequest(
  ...
  if (req.provider === "anthropic") {
    response = await callAnthropic(req, start);
  } else if (req.provider === "openai") {
    response = await callOpenAI(req, start);
  } else if (req.provider === "google") {
    response = await callGoogle(req, start);
  } else {
    throw new Error(`Unsupported provider: ${req.provider}`);
  }

  // Log usage async
  logUsage(orgId, projectId, apiKeyId, req, response).catch((e) =>
...
async function callAnthropic(req: GatewayRequest, start: number): Promise<GatewayResponse> {
...
async function callOpenAI(req: GatewayRequest, start: number): Promise<GatewayResponse> {
...
async function callGoogle(req: GatewayRequest, start: number): Promise<GatewayResponse> {
